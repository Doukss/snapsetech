...
export default function ContactForm() {
  ...
  return (
    <form action={formAction} className="rounded-[2rem] border border-white/10 bg-[linear-gradient(135deg,rgba(16,185,129,0.12),rgba(255,255,255,0.05))] p-8 backdrop-blur-sm sm:p-10">
      ...
      <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div aria-live="polite" className="max-w-md text-sm leading-6 text-white/55">
          {state.message || "Reponse sous 24h ouvrables. Nous pouvons demarrer par un cadrage rapide et sans engagement."}
        </div>
        <button
          type="submit"
          disabled={isPending}
        ...
        >
          {isPending ? "Envoi en cours..." : "Envoyer une demande"}
        </button>
      </div>
    </form>
  );
